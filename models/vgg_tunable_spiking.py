...
class LinearSpike(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, ori_input, threshold, scale, last_spike, min_thr_scale, max_thr_scale, flag=True):
        # min_thr_scale = max_thr_scale = 1.0

        ctx.save_for_backward(ori_input)
        # input = (ori_input+1.0)*threshold
        # out = input.clone()
        # ctx.min_thr_scale = min_thr_scale
        # ctx.max_thr_scale = max_thr_scale

        # out[input < min_thr_scale] = 0.0
        # out[input > max_thr_scale] = float(threshold)*scale
        # out[input < 0] = 0.0
        # out[input > threshold] = float(threshold)*scale*1.3
        out = torch.zeros_like(ori_input).cuda()
        # out[ori_input > 0] = float(threshold)
        out[ori_input > 0] = float(threshold)*scale*1.3
        # relu = nn.ReLU()
        # out = relu((ori_input+1.0)*threshold)
        # if flag:
        #     hoyer_thr = torch.mean(torch.sum(out**2, dim=(1,2,3))) / torch.mean(torch.sum(torch.abs(out), dim=(1,2,3)))
        # else: 
        #     hoyer_thr = torch.mean(torch.sum(out**2, dim=1)) / torch.mean(torch.sum(torch.abs(out), dim=1))
        # out_thr = torch.mean(out)
        # out[out<hoyer_thr] = 0.0
        # out[out>=hoyer_thr] = out_thr
        return out
    ...
    def backward(ctx, grad_output):
        ...
        # upper_bound = 0.0
        # lower_bound = -1.0
        # grad[input > lower_bound] = 1.0
        # grad[input > upper_bound] = 0.0
        ...
        return grad*grad_input, grad_input, None, None, None, None, None
...
class Threshold_relu(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, ori_input, threshold, scale, last_spike, min_thr_scale, max_thr_scale):
        input = (ori_input+1.0)*threshold
        ctx.save_for_backward(input, threshold)
        relu = nn.ReLU()
        ctx.min_thr_scale, ctx.max_thr_scale = min_thr_scale, max_thr_scale
        out = relu(input-min_thr_scale)
        out[out > max_thr_scale] = float(threshold)*scale
        # epoch = 1
        # out = relu(input-threshold*epoch*1e-3)*(threshold)/(threshold - threshold*epoch*2e-3)
        # out[out > (threshold - threshold*epoch*1e-3)] = float(threshold)
    ...
    def backward(ctx, grad_output):
        ...
        # grad_inp[input > threshold*(ctx.min_thr_scale*2 - ctx.max_thr_scale)] = 1.0
        grad_inp[input > threshold*ctx.max_thr_scale] = 0.0
        # grad_thr[input > threshold*ctx.max_thr_scale] = 1.0	

        # return grad_inp*grad_input, grad_thr*grad_threshold, None, None, None, None, None ori
        return grad_inp*grad_input, grad_threshold, None, None, None, None
...
class VGG_SNN(nn.Module):
    ...
    def __init__(self, vgg_name, activation='Linear', labels=10, timesteps=100, leak=1.0, default_threshold = 1.0, dropout=0.2, kernel_size=3, scaling_factor=0.5, dataset='CIFAR10'):
        ...
        self.scale_factor = torch.load('network_output/my_x_scale_factor')
        # with open('new_factor_scale/new_factor_x_vgg16_cifar10_1','rb') as f:
        #     self.scale_factor = pickle.load(f)
        with open('new_factor_scale/new_factor_y_vgg16_cifar10_1','rb') as f:
            self.scale_factor_y = pickle.load(f)
        with open('output/ann_min_scale_vgg16_cifar10_0.1', 'rb') as f:
            self.min_thr_scale = torch.load(f)
        with open('output/ann_max_scale_vgg16_cifar10_0.9', 'rb') as f:
            self.max_thr_scale = torch.load(f)
        # with open('output/ann_min_vgg16_cifar10_0.1', 'rb') as f:
        #     self.min_thr_scale = torch.load(f)
    ...
    def threshold_update(self, scaling_factor=1.0):
        """Update the threshold for evry conv and fc by * scaling_factor[j]
        """
        ...
        for pos in range(len(self.features)):
            if isinstance(self.features[pos], nn.Conv2d):
                ...
                self.threshold.update({'t'+str(pos): nn.Parameter((getattr(self.threshold, 't'+str(pos))*self.scale_factor[j]).clone().detach().requires_grad_(True))})###*self.scale_factor[i]
                ...
                # print('\t Layer{} : {:.2f}'.format(pos, getattr(self.threshold, 't'+str(pos))))
        ...
        for pos in range(len(self.classifier)-1):#-1
            if isinstance(self.classifier[pos], nn.Linear):
                ...
                self.threshold.update({'t'+str(prev+pos): nn.Parameter((getattr(self.threshold, 't'+str(prev+pos))*self.scale_factor[j]).clone().detach().requires_grad_(True))}) #####*self.scale_factor[i]
                ...
                # print('\t Layer{} : {:.2f}'.format(prev+pos, getattr(self.threshold, 't'+str(prev+pos))))
    ...
    def _make_layers(self, cfg):
        """ return conv part: features; fc part: classifer
        """
        layers 		= []
    ...
    def neuron_init(self, x):
        """ initialization 4 dict, all have the same size as the corresponding layer (b, c, w, h)/(b, l)

        self.mem: membrane potential

        self.spike: set t-1 for out>0

        self.spike_count: count the times of out>0 
    ...
    def layer_output_init(self, x):
        ...
        # for l in range(len(self.features)):
        # 	if isinstance(self.features[l], nn.Conv2d):
        # 		self.layer_output[l] = torch.zeros(self.batch_size, self.features[l].out_channels, self.width, self.height).cpu()
        # 	elif isinstance(self.features[l], nn.MaxPool2d):
        # 		self.width = self.width//self.features[l].kernel_size
        # 		self.height = self.height//self.features[l].kernel_size
        prev = len(self.features)
        for l in range(len(self.classifier)):
            if isinstance(self.classifier[l], nn.Linear):
                self.layer_output[prev+l] = torch.zeros(self.batch_size, self.classifier[l].out_features)
                
                
        
    def percentile(self, t, q):
        """get q% th samllest element in t
        """

        k = 1 + round(.01 * float(q) * (t.numel() - 1)) # get q% * the # of elements in t
        result = t.view(-1).kthvalue(k).values.item() # get kth samllest element in t
        return result
    


    def num_relu(self, x, min_thr_scale, max_thr_scale, threshold=None):
        min = (x<min_thr_scale-1.0).sum()
        max = (x>max_thr_scale-1.0).sum()
        # min = ((x+1.0)*threshold<min_thr_scale).sum()
        # max = ((x+1.0)*threshold>max_thr_scale).sum()
        total = x.view(-1).shape[0]
        return torch.tensor([min, total-min-max, max, total])
        

    def forward(self, x, find_max_mem=False, max_mem_layer=0, epoch=True):
        self.neuron_init(x)
        # self.layer_output_init(x)
        max_mem=0.0
        # store the num of relu in every layer in test, store the num of relu in train
        relu_num = torch.tensor([0.0, 0.0, 0.0, 0.0]) if epoch else {}
        act_out = {}
        act_mask = {}
        ...
        for t in range(self.timesteps):
            ...
            for l in range(len(self.features)):
                ...
                if isinstance(self.features[l], (nn.Conv2d)):
                    ...
                    if epoch:
                        relu_num += self.num_relu(mem_thr, self.min_thr_scale[i], self.max_thr_scale[i], getattr(self.threshold, 't'+str(l)))
                    else:
                        relu_num[l] = self.num_relu(mem_thr, self.min_thr_scale[i], self.max_thr_scale[i])
                    # print(f' in layer {l}, total num is {relu_total_num[1]}, relu num is {relu_total_num[0]}, relu_rate is {relu_total_num[0]/relu_total_num[1]*100}%, min is {self.min_thr_scale[i]}, max is {self.max_thr_scale[i]}')
                    out             = self.act_func(mem_thr,  getattr(self.threshold, 't'+str(l)), self.scale_factor_y[i], l, self.min_thr_scale[i], self.max_thr_scale[i], True)
                    if not epoch:
                        act_out[l] = out.clone()

                    out_prev 		= out * self.mask[l]  # dropout
                    i += 1       
            ...
            for l in range(len(self.classifier)-1):#-1
                ...
                if isinstance(self.classifier[l], (nn.Linear)):
                    ...
                    # self.layer_output[prev+l] = (self.mem[prev+l]/ getattr(self.threshold, 't'+str(prev+l))).clone().detach()
                    rst 				= getattr(self.threshold, 't'+str(prev+l)) * (self.max_thr_scale[i]) * (mem_thr>0).float()
                    self.mem[prev+l] 	= self.mem[prev+l]-rst
                    if epoch:
                        relu_num += self.num_relu(mem_thr, self.min_thr_scale[i], self.max_thr_scale[i], getattr(self.threshold, 't'+str(prev+l)))
                    else:
                        relu_num[prev+l] = self.num_relu(mem_thr, self.min_thr_scale[i], self.max_thr_scale[i], getattr(self.threshold, 't'+str(prev+l)))
                    out 				= self.act_func(mem_thr,  getattr(self.threshold, 't'+str(prev+l)), self.scale_factor_y[i], prev+l, self.min_thr_scale[i], self.max_thr_scale[i], False)
                    if not epoch:
        ...
        return self.mem[prev+l+1], act_out, relu_num# , self.layer_output   
        # return self.mem[prev+l+1], self.spike_count , relu_total_num# , self.layer_output   
