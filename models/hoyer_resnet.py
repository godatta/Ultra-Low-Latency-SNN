...
class BasicBlock_double(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, spike_type='sum', \
        ...
        self.bi_act1        = HoyerBiAct(num_features=inplanes, spike_type=spike_type, x_thr_scale=x_thr_scale, if_spike=if_spike)
        self.bi_conv1       = conv3x3(inplanes, planes,stride=stride)
        self.bn1            = nn.BatchNorm2d(planes)
        self.bi_act2        = HoyerBiAct(num_features=inplanes, spike_type=spike_type, x_thr_scale=x_thr_scale, if_spike=if_spike)
