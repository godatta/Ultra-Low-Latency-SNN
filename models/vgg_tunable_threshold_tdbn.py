...
class HoyerBiAct(nn.Module):
    ...
    Args:
        ...
        track_running_stats (bool): same with nn.BatchNorm2d
    """
    def __init__(self, num_features=-1, eps=1e-05, momentum=0.1, hoyer_type='sum', x_thr_scale=1.0, if_spike=True):
    ...
    def forward(self, input, epoch, min_thr_scale=0.0, max_thr_scale=1.0, layer_index=0, start_spike_layer=0):
        # calculate running estimates
        if self.training:
        ...
        input = Spike_func.apply(input, hoyer_thr, self.x_thr_scale, self.hoyer_type, (layer_index>=start_spike_layer))
        return input
...
class ThrBiAct(nn.Module):
    ...
    def forward(self, input, epoch, min_thr_scale=0.0, max_thr_scale=1.0, layer_index=0, start_spike_layer=0):
        return self.F_ThrBiAct.apply(input, epoch, min_thr_scale, max_thr_scale, layer_index, start_spike_layer)
...
class SubThr_sum(torch.autograd.Function):
    ...
    """
    @staticmethod
    def forward(ctx, input, epoch, min_thr_scale=0.0, max_thr_scale=1.0, layer_index=0, start_spike_layer=0, scores=0.6):
        ctx.save_for_backward(input)
        ctx.epoch = epoch
        ctx.min_thr_scale, ctx.max_thr_scale = min_thr_scale, max_thr_scale
        ctx.layer_index = layer_index
    ...
    def backward(ctx, grad_output):
        ...
        return grad_inp*grad_input, None, None, None, None, None, grad_score
...
class SubBiAct(nn.Module):
    ...
    def forward(self, input, epoch, min_thr_scale=0, max_thr_scale=1, layer_index=0, start_spike_layer=0):
        # out = super().forward(out, epoch, min_thr_scale, max_thr_scale, layer_index, start_spike_layer)
        out = SubThr_sum.apply(input, epoch, min_thr_scale, max_thr_scale, layer_index, start_spike_layer, self.clamped_scores)
    ...
    def __init__(self, vgg_name='VGG16', labels=10, dataset = 'CIFAR10', kernel_size=3, linear_dropout=0.1, conv_dropout=0.1, default_threshold=1.0, \
        ...
        if vgg_name == 'VGG6' and dataset!= 'MNIST':
            ...
            self.classifier = nn.Sequential(
                            ...
                            # HoyerBiAct(if_spike=(self.start_spike_layer==0), hoyer_type='sum',x_thr_scale=self.x_thr_scale),
                            ...
                            # SubBiAct(act_mode=self.act_mode, bit=1, act_size=(4096)) if self.sub_act_mask else ThrBiAct(act_mode=self.act_mode) ,
                            ThrBiAct(act_mode=self.act_mode) ,
                            # HoyerBiAct(if_spike=(self.start_spike_layer==0), hoyer_type='sum', x_thr_scale=x_thr_scale),
                            # nn.ReLU(inplace=True),
                            #nn.Dropout(0.5),
                ...
                if i+1 < len(cfg) and cfg[i+1] == 'A':
                    if self.pool_pos == 'before_relu':
                        ...
                        layers += [conv2d,
                                ...
                                HoyerBiAct(num_features=x, if_spike=(self.start_spike_layer==0), hoyer_type=self.act_mode, x_thr_scale=self.x_thr_scale)
